$(function(){
    $("#wish_button").click(function(){
        ...
        if(wcheck==0){
            ...
            $('.wishlistForm_true').css('top', - scrollHeight + 100);// 최상위 div에 현재 스크롤된값 = 보이는화면만큼 top값 추가
            $('.wishlistForm_true').css('left', 700);// 최상위 div에 현재 스크롤된값 = 보이는화면만큼 top값 추가
            $.ajax({
            ...
            $('.wishlistForm_false').css('top', - scrollHeight + 100);// 최상위 div에 현재 스크롤된값 = 보이는화면만큼 top값 추가
            $('.wishlistForm_false').css('left', 700);// 최상위 div에 현재 스크롤된값 = 보이는화면만큼 top값 추가
        	//팝업창 띄우고 확인 누르면 db삭제
            $.ajax({
